...
import { createPartnerOrganization, createPartnerPlacement, markContractPaid, registerManualInvoice, releasePartnerMarket, updateLeadStatus, updatePlacementStatus } from "./actions";
...
type ContractItem = { id:string; contract_id:string; status:string; release_reason:string|null; partner_contracts:{id:string;company_name:string;invoice_number:string|null;invoice_status:string;invoice_provider:string;fortnox_invoice_number:string|null;status:string}|null; partner_placements:{municipalities:{name:string}|null;service_categories:{name:string}|null}|null };
...
export default async function PartnerAdminPage({ searchParams }: Props) {
  ...
  const [leadResult,organizationResult,municipalityResult,serviceResult,placementResult,eventResult,contractItemResult]=await Promise.all([
    ...
    supabase.from("partner_contract_items").select("id,contract_id,status,release_reason,partner_contracts(id,company_name,invoice_number,invoice_status,invoice_provider,fortnox_invoice_number,status),partner_placements(municipalities(name),service_categories(name))").order("created_at",{ascending:false}),
  ]);
  ...
  return <main className="editor-page partner-admin">
    ...
    <section className="partner-admin-section"><div className="panel-heading"><h2>Avtalade partnerplatser</h2><span>{contractItems.length} totalt</span></div>
      <div className="partner-placement-table">{contractItems.map(item=><article key={item.id}>
        ...
        <div><small>Kund</small><strong>{item.partner_contracts?.company_name}</strong><span>Faktura {item.partner_contracts?.invoice_number??item.partner_contracts?.fortnox_invoice_number??"ska skapas"}</span></div>
        <div><small>Status</small><strong>{item.partner_contracts?.invoice_status??item.status}</strong><span>{item.partner_contracts?.invoice_provider??"manual"} · {item.partner_contracts?.status}</span></div>
        {item.partner_contracts?.invoice_status==="pending_creation"?<form action={registerManualInvoice}><input type="hidden" name="contractId" value={item.partner_contracts.id}/><input name="invoiceNumber" placeholder="Fakturanummer" required/><button type="submit">Registrera faktura</button></form>:item.partner_contracts?.invoice_status!=="paid"?<form action={markContractPaid}><input type="hidden" name="contractId" value={item.partner_contracts?.id??item.contract_id}/><button type="submit">Markera betald</button></form>:<strong>Betald</strong>}
