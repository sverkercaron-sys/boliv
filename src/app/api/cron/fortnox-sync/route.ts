...
export async function GET(request:NextRequest){
 ...
 for(const contract of contracts??[]){try{
  ...
  if(Number(result.Invoice.Balance)===0&&!result.Invoice.Cancelled){
   await Promise.all([admin.from("partner_contracts").update({status:"active",invoice_status:"paid",paid_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",contract.id),admin.from("partner_contract_items").update({status:"active"}).eq("contract_id",contract.id).eq("status","sold"),admin.from("partner_placements").update({status:"active",reserved_until:null,internal_notes:"Betald och aktiverad automatiskt",updated_at:new Date().toISOString()}).in("id",placementIds),admin.from("partner_organizations").update({status:"active",profile_published:true,updated_at:new Date().toISOString()}).eq("id",contract.organization_id)]);activated++;
